...
public class MapGraph {
	...
	public Set<GeographicPoint> getVertices() {
		HashSet<GeographicPoint> vertices = new HashSet<>();
		for (GeographicPoint a : nodes.keySet()) {
			vertices.add(a);
		}
		return vertices;
	}
	...
	public boolean addVertex(GeographicPoint location) {
		if (location != null && !nodes.containsKey(location)) {
			MapModel mapModel = new MapModel(location);
			nodes.put(location, mapModel);
			return true;
		} else {
			return false;
		}
	...
	public void addEdge(GeographicPoint from, GeographicPoint to, String roadName, String roadType, double length)
		...
		if (!nodes.containsKey(from) || !nodes.containsKey(to) || roadName == null || roadType == null || length < 0
				|| from == null || to == null) {
	...
	public List<GeographicPoint> bfs(GeographicPoint start, GeographicPoint goal,
		...
		return new BFS().bfsSearch(start, goal, nodeSearched, nodes);
	}
